...
class AtariDQNAgent(DQNBaseAgent):
    def __init__(self, config):
        ...
        self.env = gym.wrappers.AtariPreprocessing(
            self.env, noop_max=30, frame_skip=1, screen_size=84, terminal_on_life_loss=False, grayscale_obs=True)
        ...
        self.test_env = gym.wrappers.AtariPreprocessing(
            self.test_env, noop_max=30, frame_skip=1, screen_size=84, terminal_on_life_loss=False, grayscale_obs=True)
    ...
    def update_behavior_network(self):
        ...
        with torch.no_grad():
            q_next = self.target_net(next_state).max(
                dim=1).values.view(self.batch_size, 1)
