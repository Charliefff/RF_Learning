...
public:
	 ...
	 * u = error * alpha
	 * 平均分給每個feature update 後全部feat 相加
	 */
	float 
    ...
    for (state* move = after; move != after + 4; move++) {
        if (move->assign(b)) {
            ...
            for (int i = 0; i < 16; i++) {
                if (b.at(i) == 0) {
                    board tmp_2 = move->after_state();
                    board tmp_4 = move->after_state();
                    tmp_2.set(i, 1); 
					float value_2 = estimate(tmp_2);
                    tmp_4.set(i, 2);
                    float value_4 = estimate(tmp_4);
            ...
            if (num != 0) {
                move->set_value((total_value / num) + move->reward());
            } else {
	...
	void update_episode(std::vector<state>& path, float lambda = 0.5, float alpha = 0.1) const {
		float target = 0;
		float old_target = 0;
		float error ;
		for (int t = path.size() - 1; t >= 0; t--) {
			state& move = path[t];
       		state& pre_move = path[t - 1];

			error = target - estimate(move.before_state());
			// 下一輪
			old_target = target;
			target = move.reward() + lambda * old_target + update(move.before_state(), alpha * error)*(1-lambda);
		}
		error = target - estimate(path[0].before_state());
		update(path[0].before_state(), alpha * error)*(1-lambda);
	}
...
int main(int argc, const char* argv[]) {
	...
	float lambda = 0.5;
	size_t total = 800000;
	float random_num = 0.01;
	unsigned seed;
	__asm__ __volatile__ ("rdtsc" : "=a" (seed));
	info << "alpha = " << alpha << std::endl;
	info << "lambda = " << lambda << std::endl;
	info << "random_rate = " << random_num << std::endl;
	...
	tdl.load("../output/2048_TD_lamda/20000weight.bin");

	// train the model
	std::vector<state> path;
	std::vector<int> opcode_vector = {0, 1, 2, 3};
	path.reserve(20000);
	...
	for (size_t n = 1; n <= total; n++) {
		...
		while (true) {
			...
			if (x > random_num){
				...
				int index = rand() % opcode_vector.size();
				int opcode = opcode_vector[index];
				best = state(opcode);
				best.assign(b);
				opcode_vector.erase(opcode_vector.begin() + index);
				while (!best.is_valid() && opcode_vector.size() != 0)
				{
					int index = rand() % opcode_vector.size();
					int opcode = opcode_vector[index];
					best = state(opcode);
		...
		// update by TD(λ)
		tdl.update_episode(path, lambda, alpha);
		tdl.make_statistic(n, b, score, COUNT, 1000);
		
		std::string basename = "weight.bin";
		std::string fullname = "../output/2048_TD_lamda/" + std::to_string(n) + basename ;
