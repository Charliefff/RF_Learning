...
public:
	...
	void update_episode(std::vector<state>& path, float lambda = 0.5, float alpha = 0.1) const {
		...
		for (int t = path.size() - 1; t >= 0; t--) {
			...
			// 下一輪
			old_target = target;
			target = move.reward() + lambda * old_target + update(move.after_state(), alpha * error)*(1-lambda);
		}
	}
...
int main(int argc, const char* argv[]) {
	...
	tdl.load("/home/tzeshinchen/data/RF_learning/HW1/weight_state.bin");
	...
	for (size_t n = 1; n <= total; n++) {
		...
		// update by TD(0)
		// 改成TD(lambda)
		// 參考 : https://ko19951231.github.io/2021/01/01/2048/
