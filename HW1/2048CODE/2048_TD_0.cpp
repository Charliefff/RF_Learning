...
void update_episode(std::vector<state>& path, float lambda = 0.5, float alpha = 0.1) const {
    ...
    for (int t = path.size() - 1; t > 0; t--) {
        ...
        target = pre_move.reward() + update(move.before_state(), alpha * error);
    }

    error = target - estimate(path[0].before_state());
    update(path[0].before_state(), alpha * error);
}
...
int main(int argc, const char* argv[]) {
	...
	tdl.load("./output/test1/140000weight.bin");
	...
	for (size_t n = 1; n <= total; n++) {
		...
		std::string fullname = "./output/test1/" + std::to_string(n) + basename ;
