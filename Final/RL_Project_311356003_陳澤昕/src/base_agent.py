...
class TD3BaseAgent(ABC):
    ...
    def update(self):
        ...
        if self.total_time_step % self.update_freq == 0:
            self.update_target_network(
                self.target_actor_net, self.actor_net, self.tau)
            self.update_target_network(
    ...
    def train(self):
        for episode in range(self.total_episode):
            ...
            for t in range(10000):
                ...
                next_state, reward, terminates, truncates, info = self.env.step(
                    action)
                if info["wall_collision"]:
                    n_collision += 1

                self.replay_buffer.append(
                    state, action, [reward/10], next_state, [int(terminates)])
                if self.total_time_step >= self.warmup_steps:
                    self.update()
                ...
                total_reward += reward
                state = next_state
                end_progress = info['progress']

                if terminates or truncates:
                    if end_progress < start_progress:
                        movement = 1 - start_progress + end_progress
                    else:
                        movement = end_progress - start_progress
                    self.writer.add_scalar(
                        'Train/Episode Reward', total_reward, self.total_time_step)
                    print(
                        'Step: {}\tEpisode: {}\tLength: {:3d}\tTotal reward: {:.2f}\tNum of Collision: {}\tLap: {}\tStart progress: {:.2f}\tEnd progress: {:.2f}\tMovement: {:.2f}'
                        .format(self.total_time_step, episode+1, t, total_reward, n_collision, info['lap'], start_progress, end_progress, movement))
                    break

            if (episode+1) % self.eval_interval == 0:
                # save model checkpoint
                avg_score = self.evaluate()
                self.save(os.path.join(self.writer.log_dir,
    ...
    def evaluate(self):
        ...
        for episode in range(self.eval_episode):
            ...
            for t in range(10000):
                action = self.decide_agent_actions(state, mode='eval')
                next_state, reward, terminates, truncates, _ = self.test_env.step(
                    action)

                state = next_state
                if self.env_remote is not True:
                    total_reward += reward

                if terminates or truncates:
                    if self.env_remote:
                        eval_message = 'Episode: {}\tLength: {:3d}\t'.format(
                            episode+1, t)
                    else:
                        eval_message = 'Episode: {}\tLength: {:3d}\tTotal reward: {:.2f}'.format(
