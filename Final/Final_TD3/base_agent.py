...
class TD3BaseAgent(ABC):
    ...
    def train(self):
        for episode in range(self.total_episode):
            ...
            self.noise.reset()
            self.old_progess = infos['progress']
            for t in range(10000):
                ...
                if terminates or truncates:
                    final_progress = info['progress'] - self.old_progess
                    if final_progress < 0:
                        final_progress = (
                            1 - self.old_progess) + info['progress']

                    self.writer.add_scalar(
                        'Train/Episode Reward', total_reward, self.total_time_step)
                    print(
                        'Step: {}\tEpisode: {}\tLength: {:3d}\tTotal reward: {:.2f}\tProgress: {:.3f}'
                        .format(self.total_time_step, episode+1, t, total_reward, final_progress))
    ...
    def evaluate(self):
        ...
        for episode in range(self.eval_episode):
            ...
            for t in range(10000):
                ...
                next_state, _, terminates, truncates, _ = self.test_env.step(
                    action)
                total_reward += reward
                state = next_state
                if terminates or truncates:
                    print(
                        ...
                        .format(episode+1, t, total_reward * 0.02))
