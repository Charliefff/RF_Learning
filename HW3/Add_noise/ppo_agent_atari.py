...
class AtariPPOAgent(PPOBaseAgent):
    def __init__(self, config):
        ...
        self.env = gym.wrappers.AtariPreprocessing(
            self.env, 30, 1, 84, False, True, False, False)
        ...
        self.test_env = gym.wrappers.AtariPreprocessing(
            self.test_env, 30, 1, 84, False, True, False, False)
    ...
    def decide_agent_actions(self, observation, eval=False):
        ...
        observation = torch.tensor(np.asarray(
        ...
        for _ in range(self.update_count):
            for start in range(0, sample_count, self.batch_size):
                ...
                adv_train_batch = adv_train_batch.to(
                    self.device, dtype=torch.float32)
                logp_pi_train_batch = torch.from_numpy(
                    logp_pi_train_batch).squeeze()
                ...
                surr2 = torch.clamp(
                    ratio, 1 - self.clip_epsilon, 1 + self.clip_epsilon) * adv_train_batch
